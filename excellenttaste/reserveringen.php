...
class clsPage extends clsDefaultPage
{
     ...
     private function save() 
     {
          ...
          if ($reservering_id < 0)
          {
               $sql = "INSERT INTO reserveringen 
                                   (Tafel, Datum, Tijd, Klant_ID, Aantal, Verjaardag) 
                                   VALUES 
                                   ($tafel, '$datum', '$tijd', $klant_id, $aantal, $verjaardag)"; 
          }
          else
          {
               $sql = "UPDATE reserveringen  
                          SET Tafel = $tafel, Datum = '$datum', Tijd = '$tijd', Aantal = $aantal, Verjaardag = $verjaardag
     ...
     private function getKlantVars($klantId) 
     {
          if ($klantId > -1)
          {
               $sql = "SELECT ID AS KID, Naam, Email, Telefoon, Verjaardag
                         FROM klanten
                        WHERE ID = " . $klantId; 
     ...
     private function getReserveringVars($reserveringId) 
     {
          if ($reserveringId > -1)
          {
               $sql = "SELECT ID AS RID, Tafel, Datum, Tijd, Klant_ID, Aantal, Verjaardag
                        FROM reserveringen
     ...
     {    $dis_html = "disabled";
          ...
          $output = "
               <form action='?action=save' method='post' enctype='multipart/form-data'>
                    ...
                    <input type='text' name='verjaardag' id='verjaardag' $dis_html value='" . $row['verjaardag'] . "' />
     ...
     private function saveNewCustomer($klant_id) 
     {
          ...
          {    //save customer first and return klant_id
               ...
               $sql = "INSERT INTO klanten 
                                   (Naam, Email, Telefoon, Verjaardag) 
                              VALUES 
                                   (:klantnaam, :email, :telefoon, verjaardag)";
                    
               $stmt = $this->connection->prepare($sql);
               $stmt->bindParam(':klantnaam', $klantnaam);
               $stmt->bindParam(':email', $email);
               $stmt->bindParam(':telefoon', $telefoon); 
               $stmt->bindParam(':verjaardag', $verjaardag);
